...
(
   it_num smallint NOT NULL identity(1, 1),
   it_type varchar(40) NOT NULL default '',
   it_desc varchar(40) NOT NULL default '',
   it_pickups int NOT NULL default 0,
   CONSTRAINT it_num primary key (it_num)
);

CREATE INDEX it_typ ON %dbpre%items (it_type);
...
INSERT INTO %dbpre%items (it_type,it_desc) VALUES('Super Health Pack','Big Keg ' + char(39) + 'O Health');
